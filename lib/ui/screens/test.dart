...
class DetailsScreen1 extends StatefulWidget {
  ...
  const DetailsScreen1({Key key, this.id, this.list}) : super(key: key);
...
class _DetailsScreen1State extends State<DetailsScreen1> {
  ...
  Future <String> getdata() async {
    print("productsList-"+widget.list[widget.id].SubCategory);
    ...
    await db.once().then((DataSnapshot snapshot) {
      ...
      /*for (var item in result) {
        if (item['SubCategory'] == productsList[widget.id].SubCategory) {
      ...
      print("Sub1 befor=" + Sub1.length.toString());


      /*for(int i=0; i< productsList.length; i++){
        if(productsList[i].SubCategory == productsList[widget.id].SubCategory) {
          print("list sub1: "+productsList[i].SubCategory);
          products d = new products(
      ...
      Sub1.clear();
      for(int i=0; i<productsList.length; i++){
        if(productsList[i].SubCategory == widget.list[widget.id].SubCategory){
          products d = new products(
            productsList[i].Brand,
            productsList[i].Category,
            productsList[i].ApproxCost.toString(),
            ...
            children: <Widget>[
                    ...
                    child: new SingleChildScrollView(
                      child:  Column(
                        ...
                        children: <Widget>[
                          ...
                          Text(
                            "Quantity: ${widget.list[widget.id].Quantity}",
                            style:TextStyle(fontSize: 18),
                          ),
                          Text(
                            "Approximate price: ${widget.list[widget.id].ApproxCost}",
                            style:TextStyle(fontSize: 18),
                          ),
                          Text(
                            "Description: ",
              ...
              Align(
                ...
                child: RaisedButton(shape: RoundedRectangleBorder(
                  ...
                  onPressed: () =>_launchURL(widget.list[widget.id].ProductLink),
